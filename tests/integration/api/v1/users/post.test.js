...
describe("POST to /api/v1/users", () => {
  describe("Anonymous user", () => {
    test("With unique and valid data", async () => {
      const response = await fetch(`${webserver.origin}/api/v1/users`, {
        ...
        body: JSON.stringify({
          username: "guilhermeMarinho",
          email: "guimars@gmail.com",
          password: "senha123",
        }),
      });
      ...
      const userInDatabase = await user.findOneByUsername("guilhermeMarinho");
  ...
  describe("Default user", () => {
    test("With unique and valid data", async () => {
      ...
      const user1SessionOject = await orchestrator.createSession(user1.id);

      const user2Response = await fetch(`${webserver.origin}/api/v1/users`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Cookie: `session_id=${user1SessionOject.token}`,
        },
        body: JSON.stringify({
          username: "usueriologado",
          email: "usueriologado@xedit.com",
          password: "senha123",
        }),
      });
